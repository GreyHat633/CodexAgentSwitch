...
public sealed class BootstrapperServiceTests
{
    ...
    {
        var service = Service(new(build is 19045 ? new Version(10, 0, 19045) : new Version(10, 0, build), architecture), new(new(1, 8), Architecture.X64, "test"));
    ...
    [Fact] public void Older_runtime_is_version_mismatch() => Assert.True(Service(new(new(10, 0, 19045), Architecture.X64), new(new(1, 7), Architecture.X64, "test")).Inspect().RuntimeVersionMismatch);
    [Fact] public void Matching_x64_runtime_is_accepted() => Assert.True(Service(new(new(10, 0, 22621), Architecture.X64), new(new(1, 8), Architecture.X64, "test")).Inspect().RuntimePresent);
    [Fact] public void Arm64_runtime_does_not_match() => Assert.False(Service(new(new(10, 0, 22621), Architecture.X64), new(new(1, 8), Architecture.Arm64, "test")).Inspect().RuntimePresent);
    ...
    public void Parsed_18_x64_package_is_accepted_and_17_is_mismatch()
    {
        ...
        Assert.True(Service(new(new(10, 0, 19045), Architecture.X64), match).Inspect().RuntimePresent);
    ...
    public void Current_win10_runtime_inventory_detects_18_x64_when_integration_is_enabled()
    {
        ...
        Assert.True(status.RuntimePresent, status.Message);
    ...
    private static BootstrapperService Service(OsSnapshot os, RuntimeInstallation? runtime) => new(
        new FakeOs(os),
        new FakeInventory(runtime is null ? [] : [runtime]),
        new FakeInstaller(null),
        new FakeLauncher());
