...
{
    [Fact]
    public void Zen_credential_policy_resolves_the_stable_api_key_reference()
    {
        ...
        Assert.True(ProviderCredentialPolicy.UsesApiKey(zen));
        Assert.Equal(OpenCodeZenCatalog.CredentialReference, ProviderCredentialPolicy.ResolveReference(zen));
    ...
    [Fact]
    public async Task Zen_is_listed_before_enablement_and_reports_missing_api_key()
    {
        ...
        Assert.Equal(2, credentials.ExistsCalls);
        Assert.Contains(OpenCodeZenCatalog.CredentialReference, credentials.References);
        Assert.Contains("已停用", zen.Status);
    }

    [Fact]
    public async Task Zen_refresh_discovers_models_anonymously_even_when_api_key_is_missing()
    {
        var repository = new MemoryRepository([]);
        var registry = Create(repository, (_, _) => Task.FromResult<IReadOnlyList<string>>(["vendor/zen-new"]));
        var refreshed = await registry.RefreshAsync("opencode-zen");

        Assert.Contains(refreshed.Models, model => model.Id == "vendor/zen-new");

        Assert.Equal(ProviderAuthState.Missing, refreshed.AuthState);
        Assert.False(refreshed.RefreshFailed);
    }
    ...
    public async Task Refresh_failure_keeps_saved_model_and_save_roundtrip_keeps_raw_id()
    {
        ...
        var registry = Create(repository, (_, _) => throw new InvalidOperationException("catalog unavailable"));
    ...
    private static ProviderRegistry Create(
        ...
        new(repository, new FakeClient(models), new Credentials(), new Clock());
