...
{
    public static bool UsesApiKey(ProviderConfiguration provider) => provider.Kind != ProviderKind.NativeCodex;
...
public sealed class ProviderRegistry(
    ...
    IClock clock) : IProviderRegistry
{
    ...
    private async Task<ProviderAuthResult> ReadAuthAsync(ProviderConfiguration provider, CancellationToken cancellationToken)
    {
        ...
        var reference = ProviderCredentialPolicy.ResolveReference(provider);
        var configured = reference is not null
            && await credentials.ExistsAsync(reference, cancellationToken);
        return configured
