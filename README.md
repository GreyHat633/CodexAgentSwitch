...
- 用户已经在项目根级显式设置该值时，用户值优先，CAS 不覆盖。
- 只修改已选择的 CAS 项目，不改用户级 Codex 配置，也不改未选择项目。
- CAS 只配置阈值；真正的自动压缩仍由 Codex 原生执行。
- 新建或重新加载项目线程后生效。
- 原 TOML 内容、配置备份和恢复流程保持不变；歧义配置会失败关闭。

详情见 [0.2.7.2 发布说明](docs/release-notes-0.2.7.2.md)。0.2.7.1 的冻结原因与回退边界记录在 [维护说明](docs/maintenance/0.2.7.1-frozen.md) 中。
