...
此前“仅 435 MiB”的判断不完整，只统计了当时已经移入 E 盘隔离区的部分。当前能够逐项确认的 Codex Agent Switch 开发、发布与启动物理 C 盘写入为 **1,781,483,545 字节（1,698.96 MiB / 1.659 GiB）**：
...
`C:\Users\GreyHat\.dotnet` 经复核原本就是指向 `E:\Java\VibeCoding\附件\Agent\.dotnet` 的 Junction，其中 8,524 字节新增状态不属于 C 盘物理占用，已从总量中剔除。瞬时 NuGet 下载暂存文件已经消失，因此历史峰值只可判定为“大于 1.659 GiB”，无法再伪造一个精确峰值。
