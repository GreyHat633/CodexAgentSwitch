...
Extract `CodexAgentSwitch-Setup-Bundle-win10-x64.zip` to an E-drive folder, keep its three files together, and run `CodexAgentSwitch.Setup.exe`. The setup validates the adjacent payload ZIP against its SHA-256 before changing the target. On this host the default target is `E:\Apps\Codex Agent Switch`.
...
`CodexAgentSwitch-compact-runtime-win10-x64.zip` contains the framework-dependent WinUI app, the independent self-contained WinForms bootstrapper, and Microsoft’s signed x64 `WindowsAppRuntimeInstall-x64.exe`. Start `CodexAgentSwitch.Bootstrapper.exe`; it checks Win10 build/architecture and actual HKCU/HKLM App Runtime 1.8 package registration. It never downloads or launches the Runtime installer without explicit confirmation.
